...
const OrderDetails: React.FC<{ orderId: string }> = React.memo(({ orderId }) => {
    ...
    if (loading) return <div className="p-4 flex justify-center"><Loader size="sm" text="Loading items..." /></div>;
            ...
            <div className="overflow-x-auto">
                <table className="w-full bg-white text-sm">
                    <thead className="bg-slate-50 text-slate-700 uppercase font-semibold text-xs border-b">
                        <tr>
                            <th className="px-4 py-3 font-semibold text-contentSecondary">Product</th>
                            <th className="px-4 py-3 font-semibold text-contentSecondary text-center">Delivered</th>
                            <th className="px-4 py-3 font-semibold text-contentSecondary text-center">Returned</th>
                            <th className="px-4 py-3 font-semibold text-contentSecondary text-right">Unit Price</th>
                            <th className="px-4 py-3 font-semibold text-contentSecondary text-right">Subtotal</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((item) => (
                            <tr key={item.id} className={`border-b border-slate-100 last:border-none ${item.isFreebie ? 'bg-green-50' : ''}`}>
                                <td className="px-4 py-3">{item.skuName} {item.isFreebie && <Gift size={12} className="inline ml-1 text-green-700" />}</td>
                                <td className="px-4 py-3 text-center">{item.quantity}</td>
                                <td className="px-4 py-3 text-center text-red-600">{item.returnedQuantity > 0 ? item.returnedQuantity : '-'}</td>
                                <td className="px-4 py-3 text-right">{formatIndianCurrency(item.unitPrice)}</td>
                                <td className="px-4 py-3 font-semibold text-right">{formatIndianCurrency(item.quantity * item.unitPrice)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
const EditDistributorModal: React.FC<{
    ...
    const { register, handleSubmit, formState: { errors, isValid }, watch, setValue, getValues } = useForm<Distributor>({
        mode: 'onBlur',
        defaultValues: distributor,
    });
    const [loading, setLoading] = useState(false);
    const watchedAsmName = watch('asmName');
    ...
    const executiveUsers = useMemo(() => {
        ...
        const selectedAsm = users.find(u => u.username === watchedAsmName);
        if (!selectedAsm) {
            return [];
        }
    ...
    useEffect(() => {
        const currentExec = getValues('executiveName');
        const isStillValid = executiveUsers.some(e => e.username === currentExec);
        if (!isStillValid) {
            setValue('executiveName', '');
        }
    }, [watchedAsmName, executiveUsers, setValue, getValues]);
                ...
                <form onSubmit={handleSubmit(handleSaveEdit)} className="overflow-y-auto">
                    <div className="p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <Select label="Executive" {...register('executiveName')} disabled={!watchedAsmName}><option value="">None</option>{executiveUsers.map(u => <option key={u.id} value={u.username}>{u.username}</option>)}</Select>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
const KPIStatCard: React.FC<{ title: string; value: string; children?: React.ReactNode }> = React.memo(({ title, value, children }) => (
...
const DistributorDetailsPage: React.FC = () => {
    ...
    const { items: sortedTransactions, requestSort: requestTxSort, sortConfig: txSortConfig } = useSortableData<EnrichedWalletTransaction>(transactions, { key: 'date', direction: 'descending' });
    const { items: sortedOrders, requestSort: requestOrderSort, sortConfig: orderSortConfig } = useSortableData<Order>(orders, { key: 'date', direction: 'descending' });
    const { items: sortedReturns, requestSort: requestReturnSort, sortConfig: returnSortConfig } = useSortableData<EnrichedOrderReturn>(returns, { key: 'initiatedDate', direction: 'descending' });
    ...
    if (loading && !distributor) return <Loader fullScreen text="Loading distributor details..." />;
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                <div className="flex flex-col md:flex-row justify-between items-center border-b border-border p-4 gap-4">
                    <nav className="flex space-x-6 overflow-x-auto" aria-label="Tabs">
                        <button onClick={() => setActiveTab('orders')} className={`flex items-center gap-2 py-3 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === 'orders' ? 'border-primary text-primary' : 'border-transparent text-contentSecondary hover:text-content hover:border-slate-300'}`}><ShoppingCart size={16} /> Orders</button>
                        ...
                        <button onClick={() => setActiveTab('assignments')} className={`flex items-center gap-2 py-3 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === 'assignments' ? 'border-primary text-primary' : 'border-transparent text-contentSecondary hover:text-content hover:border-slate-300'}`}><Sparkles size={16} /> Assignments</button>
                    </nav>
                    <div className="min-w-[250px]">
                        <DateRangePicker label="Filter Date" value={dateRange} onChange={setDateRange} />
                    </div>
                </div>
                                ...
                                <table className="w-full text-left min-w-[700px] text-sm">
                                    <thead className="bg-slate-50 text-slate-700 uppercase font-semibold text-xs h-12 border-b">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 font-semibold text-contentSecondary text-right">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {sortedOrders.map(o => {
                                            const twoDaysInMs = 2 * 24 * 60 * 60 * 1000;
                                            const isReturnWindowOpen = (new Date().getTime() - new Date(o.date).getTime()) < twoDaysInMs;
                                            return (
                                                            ...
                                                            <div className="flex justify-end gap-1">
                                                                {o.status === 'Pending' ? (
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <Button size="sm" variant="secondary" onClick={() => navigate(`/ewaybill/${o.id}`)}>E-Way Bill</Button>
                                                                        <Button size="sm" variant="secondary" onClick={() => setReturningOrder(o)} disabled={!isReturnWindowOpen} title={!isReturnWindowOpen ? 'Return window closed (2 days after billing)' : 'Initiate Return'}><CornerUpLeft size={14} /></Button>
                                                                    </>
                                                                )}
                                                            </div>
                                                        </td>
                                ...
                                <table className="w-full text-left min-w-[700px]">
                                    ...
                                    <tbody>
                                        {sortedTransactions.map(t => (
                                            <tr key={t.id} className="border-b border-border last:border-0 text-sm hover:bg-slate-50 transition-colors">
                                                <td className="px-4 py-3 text-xs text-contentSecondary whitespace-nowrap">{formatDateTimeDDMMYYYY(t.date)}</td>
                                                <td className="px-4 py-3 capitalize font-medium">{t.type.replace(/_/g, ' ').toLowerCase()}</td>
                                                <td className="px-4 py-3 text-xs">{t.paymentMethod || (t.orderId ? `Order: ${t.orderId}` : null)}</td>
                                                <td className="px-4 py-3 text-xs text-contentSecondary italic max-w-[150px] truncate" title={t.remarks || undefined}>{t.remarks || '—'}</td>
                                                <td className={`px-4 py-3 font-semibold text-right ${t.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>{t.amount >= 0 ? `+${formatIndianCurrency(t.amount)}` : formatIndianCurrency(t.amount)}</td>
                                                <td className="px-4 py-3 font-bold text-right text-slate-700">{formatIndianCurrency(t.balanceAfter)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                ...
                                <table className="w-full text-left min-w-[700px] text-sm">
                                    <thead className="bg-slate-50 text-slate-700 uppercase font-semibold text-xs h-12 border-b">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 font-semibold text-contentSecondary">Details</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {sortedReturns.map(r => (
                                            <tr key={r.id} className="border-b border-border last:border-0">
                                                ...
                                                <td className="p-2 text-xs text-contentSecondary">{r.skuDetails.map(i => `${i.quantity} x ${i.skuName}`).join(', ')}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        <thead className="bg-slate-50 text-slate-700 uppercase font-semibold text-xs h-10 border-b"><tr><th className="px-4 py-2 font-semibold text-left">Product</th><th className="px-4 py-2 font-semibold text-right">Price</th><th className="px-4 py-2 font-semibold text-left">Source</th></tr></thead>
                                        <tbody>
                                            {effectivePricing.map(p => (
                                                <tr key={p.skuName} className={`border-b border-slate-100 last:border-0 ${p.isSpecial ? 'bg-yellow-50' : 'hover:bg-slate-50'} transition-colors`}>
                                                    <td className="px-4 py-3 font-medium text-slate-700">{p.skuName}</td>
                                                    <td className={`px-4 py-3 font-semibold text-right ${p.isSpecial ? 'text-yellow-800' : ''}`}>{formatIndianCurrency(p.price)}</td>
                                                    <td className="px-4 py-3 text-xs text-contentSecondary">{p.source}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
